 ...
 * Если в коллекции уже есть объект с подобным ключом, то он перезаписывается. Если ключа еще
 * не было в коллекции, то метод возвращает null, иначе - предыдущее значение которое
 ...
 * такой ключ уже в отображении.
 * Необходимо перебрать весь список и добавить пользователей в отображение, при этом в качестве
 * ключа использовать id пользователя, значения - объект типа User. В задании необходимо
 * использовать методы, описанные в выше.
 */
